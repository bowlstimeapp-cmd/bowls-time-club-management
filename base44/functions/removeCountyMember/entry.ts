...
export default async function(req) {
  try {
    ...
    if(m.role==='admin'&&m.status==='approved'){const admins=await base44.asServiceRole.entities.CountyMembership.filter({county_id:m.county_id,role:'admin',status:'approved'});if(admins.length<=1)return Response.json({error:'Cannot remove the last remaining administrator'},{status:409});}
    await base44.asServiceRole.entities.CountyMembership.delete(membershipId); return Response.json({success:true,membershipId});
  } catch(e){return Response.json({error:e.message||'Failed to remove county member'},{status:e.status||500});}
}