 ...
 * Only an approved club admin (ClubMembership role='admin') or platform admin can update a club.
 */
Deno.serve(async (req) => {
  try {
    ...
    // Verify caller is an approved club admin for this club OR a platform admin
    ...
    // Whitelist of fields a club admin is allowed to update
    // Platform-only fields (modules, sms_monthly_allowance) are excluded
    const allowedFields = [
