...
Deno.serve(async (req) => {
  try {
    ...
    // colWidths defined here so it's available throughout the card rendering
    ...
    for (const card of scorecards) {
      ...
      // Player positions — full width rows, text centered on the card
      ...
      ['1', '2', '3', 'Skip'].forEach((pos, idx) => {
        ...
        // Centered across the full card width — same anchor as 'Vs'
        doc.text(pos, x + cardWidth / 2, rowY + posRowHeight - 1.5, { align: 'center' });
      });
