...
{editingRotaTeam && (() => {
  ...
  const unavailability = liveEditingTeam.player_unavailability || {};
...
const isHome = fixture.home_team_id === liveEditingTeam.id;
                        ...
                        return (
                          <tr key={fixture.id} className={isHighlighted ? 'bg-red-50' : ''}>
                            ...
                            <td className={`border p-2 whitespace-nowrap ${isHighlighted ? 'border-red-300' : ''}`}>
                              {format(parseISO(fixture.match_date), 'd MMM')}
                            </td>
                            <td className={`border p-2 text-center ${isHighlighted ? 'border-red-300' : ''}`}>
                              {fixture.rink_number}
                            </td>
                            {players.map(player => {
                              const isUnavailable = (unavailability[player] || []).includes(fixture.match_date);
                              return (
                               <td key={player} className={`border p-2 text-center ${isHighlighted ? 'border-red-300' : ''}`}>
                                 {isUnavailable ? (
