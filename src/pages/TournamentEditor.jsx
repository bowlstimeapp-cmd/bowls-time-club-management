...
export default function TournamentEditor() {
  ...
  const [playerTeams, setPlayerTeams] = useState([]); // for pairs/triples/fours
  ...
  const getMemberName = (entry) => {
    if (!entry) return '';
    // entry may be pipe-separated emails for teams
  ...
  const getDrawEntries = () => {
    ...
    // For pairs/triples/fours: validate all teams are complete
    const completedTeams = playerTeams.filter(t => t.length === teamSize);
    return completedTeams.map(t => t.join('|'));
  };
  ...
  const generateBracket = () => {
    ...
    if (entries.length < 2) {
      if (teamSize > 1) {
        toast.error(`Complete at least 2 teams of ${teamSize} players`);
      } else {
    ...
    const entryCount = shuffled.length;
    ...
    const byeInterval = entryCount > 0 ? Math.floor(bracketSize / (byeCount || 1)) : 0;
  ...
  const randomizeGroups = () => {
    ...
    for (let i = 0; i < numGroups; i++) {
      newGroups[String.fromCharCode(65 + i)] = [];
    }
  ...
  const generateRoundRobin = () => {
      ...
      for (let round = 0; round < totalRounds; round++) {
        ...
        for (let i = 0; i < teamList.length / 2; i++) {
          ...
          if (team1.id !== 'bye' && team2.id !== 'bye') {
            roundMatches.push({ team1_id: team1.id, team2_id: team2.id, group: groupLetter });
          }
        }
    ...
    for (let i = 0; i < maxRounds; i++) {
      const roundMatches = [];
      Object.values(roundsData).forEach(rounds => { if (rounds[i]) roundMatches.push(...rounds[i]); });
    ...
    mergedRounds.forEach((roundMatches, roundIdx) => {
      roundMatches.forEach(match => {
        ...
        for (let rink = 1; rink <= rinksAvailable; rink++) {
          if (!teamRinkUsage[match.team1_id].has(rink) && !teamRinkUsage[match.team2_id].has(rink)) {
            assignedRink = rink; break;
          }
        }
        if (!assignedRink) {
        ...
        allFixtures.push({
          ...
          group: match.group, rink: assignedRink,
    ...
    else if (totalQualifiers === 2) { knockoutStage.push({ round: 'Final', matches: 1 }); }
    const knockoutFixturesList = [];
    let knockoutId = 0;
    knockoutStage.forEach(stage => {
      for (let i = 0; i < stage.matches; i++) {
        knockoutFixturesList.push({ id: `knockout_${knockoutId++}`, round: stage.round, team1_id: null, team2_id: null, rink: null, team1_score: null, team2_score: null, winner_id: null });
      }
    });
  ...
  const teamSize = FORMAT_SIZES[compFormat];
  const completedTeams = playerTeams.filter(t => t.length === teamSize);
  const allPlayersAssigned = compFormat !== 'singles' && selectedPlayers.length > 0 && playerTeams.flat().length === selectedPlayers.length && completedTeams.length === playerTeams.length;
                  ...
                  <>
                    ...
                    <div>
                      <Label className="flex items-center gap-2 mb-2">
                        ...
                        Select Players ({selectedPlayers.length}{compFormat !== 'singles' ? ` selected` : '/32'})
                      </Label>
                    ...
                    {/* Team builder for pairs/triples/fours */}
                    {compFormat !== 'singles' && selectedPlayers.length > 0 && (
                      <div>
                        <Label className="flex items-center gap-2 mb-2">
                          Group into {compFormat} ({teamSize} per team)
                          {allPlayersAssigned && (
                            <Badge className="bg-emerald-100 text-emerald-700 border-emerald-200 text-xs">All assigned</Badge>
                          )}
                        </Label>
                        <PlayerTeamBuilder
                          players={selectedPlayers}
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          setTeamCount(val);
                          if (val === 8) { setNumGroups(2); setTeamsPerGroup(4); }
                          else if (val === 12) { setNumGroups(3); setTeamsPerGroup(4); }
                          else if (val === 16) { setNumGroups(4); setTeamsPerGroup(4); }
                        }}
                        className="w-full h-9 rounded-md border border-input bg-transparent px-3 py-1 text-sm"
                      >
