...
export default function TournamentEditor() {
  ...
  const { data: existingTournament } = useQuery({
    ...
    queryFn: async () => {
      const ts = await base44.entities.ClubTournament.filter({ id: tournamentId });
      return ts[0];
    },
      ...
      <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-emerald-50 flex items-center justify-center">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center p-8">
          ...
          <Link to={createPageUrl('ClubTournaments') + `?clubId=${clubId}`}>
            <Button className="bg-emerald-600 hover:bg-emerald-700">Back to Tournaments</Button>
          </Link>
        </motion.div>
      </div>
    );
  ...
  const generateBracket = () => {
    ...
    if (entries.length < 2) {
      toast.error(teamSize > 1 ? `Complete at least 2 teams of ${teamSize} players` : 'Select at least 2 players');
      return;
    }
  ...
  const randomizeGroups = () => {
    ...
    shuffled.forEach((team, idx) => {
      const groupLetter = String.fromCharCode(65 + Math.floor(idx / teamsPerGroup));
  ...
  const generateRoundRobin = () => {
    ...
    if (matchesPerRound > rinksAvailable) {
      toast.error(`Not enough rinks! Need ${matchesPerRound} per round, but only ${rinksAvailable} available`);
      return;
    }
    const allFixtures = [];
    const roundsData = {};
    Object.entries(groups).forEach(([groupLetter, teamIds]) => {
      ...
      for (let round = 0; round < totalRounds; round++) {
        ...
        for (let i = 0; i < teamList.length / 2; i++) {
          const t1 = teamList[i], t2 = teamList[teamList.length - 1 - i];
          if (t1.id !== 'bye' && t2.id !== 'bye') roundMatches.push({ team1_id: t1.id, team2_id: t2.id, group: groupLetter });
        }
        rounds.push(roundMatches);
        const temp = teamList[1];
        for (let i = 1; i < teamList.length - 1; i++) teamList[i] = teamList[i + 1];
    ...
    for (let roundIdx = 0; roundIdx < maxRounds; roundIdx++) {
      ...
      roundMatches.forEach(match => {
        ...
        for (let rink = 1; rink <= rinksAvailable; rink++) {
          if (!teamRinkUsage[match.team1_id].has(rink) && !teamRinkUsage[match.team2_id].has(rink)) { assignedRink = rink; break; }
        }
        if (!assignedRink) {
          let minUsage = Infinity;
          for (let rink = 1; rink <= rinksAvailable; rink++) {
            const usage = (teamRinkUsage[match.team1_id].has(rink) ? 1 : 0) + (teamRinkUsage[match.team2_id].has(rink) ? 1 : 0);
            if (usage < minUsage) { minUsage = usage; assignedRink = rink; }
          }
        }
    ...
    knockoutStage.forEach(stage => {
      for (let i = 0; i < stage.matches; i++) knockoutList.push({ id: `knockout_${knockoutId++}`, round: stage.round, team1_id: null, team2_id: null, rink: null, team1_score: null, team2_score: null, winner_id: null });
    });
  ...
  const handleSave = async (publish = false) => {
    ...
    const data = {
      ...
      ...(tournamentType === 'knockout' ? {
        ...
        teams, rinks_available: rinksAvailable, num_groups: numGroups,
        teams_per_group: teamsPerGroup, qualifiers_per_group: qualifiersPerGroup,
        groups, fixtures, knockout_fixtures: knockoutFixtures
      }),
    ...
    if (tournamentId) {
      ...
      if (publish) navigate(createPageUrl('ClubTournaments') + `?clubId=${clubId}`);
    } else {
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-emerald-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <Link to={createPageUrl('ClubTournaments') + `?clubId=${clubId}`} className="inline-flex items-center text-gray-600 hover:text-gray-900 mb-4">
            <ArrowLeft className="w-4 h-4 mr-2" />
            Back to Tournaments
          </Link>
          <h1 className="text-3xl font-bold text-gray-900">{tournamentId ? 'Edit Tournament' : 'New Tournament'}</h1>
        </motion.div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g., Club Championship 2024" />
                </div>

                <div>
                  <Label>Tournament Type</Label>
                  <select value={tournamentType} onChange={(e) => setTournamentType(e.target.value)} className="w-full h-9 rounded-md border border-input bg-transparent px-3 py-1 text-sm">
                ...
                {tournamentType === 'knockout' ? (
                  <>
                    ...
                    {compFormat !== 'singles' && selectedPlayers.length > 0 && (
                      <div>
                        <Label className="flex items-center gap-2 mb-2">
                          Group into {compFormat} ({teamSize} per team)
                          {completedTeams.length >= 2 && (
                            <Badge className="bg-emerald-100 text-emerald-700 border-emerald-200 text-xs">{completedTeams.length} teams ready</Badge>
                          )}
                        </Label>
                        <PlayerTeamBuilder
                          players={selectedPlayers}
                    ...
                    <Button
                      ...
                      variant="outline"
                      className="w-full"
                      disabled={compFormat === 'singles' ? selectedPlayers.length < 2 : completedTeams.length < 2}
                    >
                  ...
                  <>
                    <div>
                      ...
                      <select value={teamCount} onChange={(e) => { const val = Number(e.target.value); setTeamCount(val); if (val === 8) { setNumGroups(2); setTeamsPerGroup(4); } else if (val === 12) { setNumGroups(3); setTeamsPerGroup(4); } else if (val === 16) { setNumGroups(4); setTeamsPerGroup(4); } }} className="w-full h-9 rounded-md border border-input bg-transparent px-3 py-1 text-sm">
                        {[4, 6, 8, 12, 16, 20, 24, 32].map(n => <option key={n} value={n}>{n} Teams</option>)}
                      </select>
                    </div>
                    <div>
                      <Label>Number of Groups</Label>
                      <select value={numGroups} onChange={(e) => setNumGroups(Number(e.target.value))} className="w-full h-9 rounded-md border border-input bg-transparent px-3 py-1 text-sm">
                        {[2, 3, 4, 5, 6, 8].map(n => <option key={n} value={n}>{n} Groups</option>)}
                      </select>
                    </div>
                    <div>
                      <Label>Teams Per Group</Label>
                      <select value={teamsPerGroup} onChange={(e) => setTeamsPerGroup(Number(e.target.value))} className="w-full h-9 rounded-md border border-input bg-transparent px-3 py-1 text-sm">
                        {[3, 4, 5, 6, 8].map(n => <option key={n} value={n}>{n} Teams</option>)}
                      </select>
                    </div>
                    <div>
                      ...
                      <div className="max-h-48 overflow-y-auto border rounded-lg p-2 space-y-2">
                        {teams.map((team, idx) => (
                          <Input key={team.id} value={team.captain} onChange={(e) => { const t = [...teams]; t[idx].captain = e.target.value; setTeams(t); }} placeholder={`Team ${idx + 1} Captain`} className="text-sm" />
                        ))}
                      </div>
                    </div>
                    <div>
                      <Label>Number of Rinks</Label>
                      <select value={rinksAvailable} onChange={(e) => setRinksAvailable(Number(e.target.value))} className="w-full h-9 rounded-md border border-input bg-transparent px-3 py-1 text-sm">
                        {[2, 3, 4, 5, 6, 7, 8].map(n => <option key={n} value={n}>{n} Rinks</option>)}
                      </select>
                    </div>
                    <div>
                      <Label>Qualifiers Per Group</Label>
                      <select value={qualifiersPerGroup} onChange={(e) => setQualifiersPerGroup(Number(e.target.value))} className="w-full h-9 rounded-md border border-input bg-transparent px-3 py-1 text-sm">
                        <option value={1}>1 Per Group</option>
                        <option value={2}>2 Per Group</option>
                      </select>
                    </div>
                    <Button onClick={randomizeGroups} variant="outline" className="w-full" disabled={teams.some(t => !t.captain.trim())}>
                      <Grid3x3 className="w-4 h-4 mr-2" />Randomize Groups
                    </Button>
                    <Button onClick={generateRoundRobin} variant="outline" className="w-full" disabled={!groups}>
                      <Shuffle className="w-4 h-4 mr-2" />Generate Round Robin
                    </Button>
                  </>
                )}

                <div className="pt-4 space-y-2 border-t">
                  <Button onClick={() => handleSave(false)} variant="outline" className="w-full" disabled={isSaving || !name.trim()}>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="lg:col-span-2">
            {tournamentType === 'knockout' ? (
              ...
              <div className="space-y-6">
                          ...
                          <div key={letter} className="border rounded-lg p-4">
                            ...
                            <div className="space-y-1">
                              {teamIds.map(teamId => {
                                ...
                                return <div key={teamId} className="text-sm">{team?.name}: {team?.captain}</div>;
                              })}
                            </div>
                          </div>
                        ))}
                ...
                {fixtures.length > 0 && (
                  <Card>
                    <CardHeader><CardTitle>Group Stage Fixtures</CardTitle></CardHeader>
                    <CardContent>
                ...
                {knockoutFixtures.length > 0 && (
                  <Card>
                    <CardHeader><CardTitle>Knockout Stage</CardTitle></CardHeader>
                    <CardContent>
                      <div className="space-y-4">
                            ...
                            <div key={round}>
                              ...
                              <div className="space-y-2">
                                {roundFixtures.map((fixture, idx) => (
                                  <div key={fixture.id} className="border rounded p-2 text-sm">Match {idx + 1}: TBD vs TBD</div>
                                ))}
                              </div>
                            </div>
                          );
